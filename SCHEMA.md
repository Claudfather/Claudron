...
**`INDEX.md` is additionally a GENERATED file** (engine 0.5.0, `claudron index
--navigation`, [#155](https://github.com/Claudfather/Claudron/issues/155)). Being
skipped by the note walk says only that the engine does not *read* it as a note;
from 0.5.0 the engine also *writes* it, deriving each directory's file from the
index rather than from the file's previous text. Every generated file opens with
`<!-- generated by claudron index --navigation; do not hand-edit -->`, and a file
carrying that header is not a human's to edit — the next regeneration replaces
its derived section. This is what removes `INDEX.md` from the set of
hand-appended files: six of the nine merge conflicts in the 2026-09-21 vault
repair were `INDEX.md`, and a derived artifact is regenerated, never merged.

A regeneration is **not** a blanket overwrite. An entry already in the file that
the index does not know about is split on whether its **target exists**:
preserved verbatim under a marked section if it does (it is somebody's
hand-written knowledge), dropped if it does not (it is dangling and navigates
nowhere). A third outcome applies to a FIELD rather than an entry: where the
index knows the entry but the note carries no `description:`, the description
already in the file is **carried** rather than dropped — the note's frontmatter
wins whenever it has one, and hand-written prose is never deleted merely because
the frontmatter is silent. All three outcomes are reported on every run.
`README.md` and `CLAUDE.md` remain prose and are never generated.
