...
| **A killed replay is cleaned up; a conflict is not** | `sync` tells the two apart and only ever repairs the first. A rebase **killed** by the per-op timeout holds no unmerged paths and no `stopped-sha` — nothing stopped it deliberately and there is nothing to resolve — so it is aborted and the tree restored, with `detail` saying so. A rebase stopped on a **conflict** is left exactly as before: its markers are a human's work in progress, and erasing them is worse than the wedge. Both facts must say "killed" before anything is aborted; an ambiguous state is treated as a conflict. |
| **`sync` never leaves markers in the vault** (`CLAUDRON_SYNC_WORKTREE=1`, #158) | Integration happens in a throwaway worktree, so a conflict or a kill leaves the live tree **byte-identical** — no `rebase-merge/`, no conflict markers, nothing half-checked-out. The live tree moves only by `merge --ff-only` or by one `reset --keep` to a tip built elsewhere. A conflict is reported **by path** in `detail`, and `quarantined` names notes that **could not be integrated** rather than notes carrying markers; resolve by editing the note on the default branch and re-running `sync`. Why it matters: a stopped in-place rebase checks out the *other* side's files, and on 2026-09-09 that made a fleet's manifest, charter and projects file absent from disk — a review three weeks later reported them as never having existed and the fleet re-drafted both. |
